...
from gim.core.models import Issue, Commit, IssueCommits


class OldIssueCommits(models.Model):

    class Meta:
        db_table = u'core_issue_commits'
...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding model 'IssueCommits'
        db.create_table(u'core_issuecommits', (
            (u'id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
            ('issue', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['core.Issue'])),
            ('commit', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['core.Commit'])),
            ('deleted', self.gf('django.db.models.fields.BooleanField')(default=False)),
        ))
        db.send_create_signal(u'core', ['IssueCommits'])

        if not db.dry_run:
            # save data from old table to new table
            IssueCommits.objects.bulk_create([
                IssueCommits(
                    issue_id=data[0],
                    commit_id=data[1],
                    deleted=False
                )
                for data in OldIssueCommits.objects.values_list('issue_id', 'commit_id')
            ])

        # Removing M2M table for field commits on 'Issue'
    ...
    def backwards(self, orm):
        ...
        if not db.dry_run:
            # save data from old table to new table
            OldIssueCommits.objects.bulk_create([
                OldIssueCommits(
                    issue=data[0],
                    commit_id=data[1],
                )
                for data in IssueCommits.objects.filter(deleted=False).values_list('issue_id', 'commit_id')
            ])

        # Deleting model 'IssueCommits'
        db.delete_table(u'core_issuecommits')
    ...
    models = {
        ...
        u'core.issuecommits': {
            ...
            'commit': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['core.Commit']"}),
            'deleted': ('django.db.models.fields.BooleanField', [], {'default': 'False'}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
