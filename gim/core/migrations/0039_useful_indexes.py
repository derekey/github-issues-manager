...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        if not db.dry_run and db.backend_name == 'postgres':
            # Repository.fetch_updated_prs
            sql = """
            CREATE INDEX prs_to_update
            ON core_issue(
                updated_at,
                repository_id,
                is_pull_request,
            ...
            WHERE
                is_pull_request IS TRUE
                AND
                (
                    pr_fetched_at IS NULL
                    OR
                    pr_fetched_at < updated_at
                    OR
                    (
                        state = 'open'
                        AND
                        ...
                        AND
                        (
                            mergeable_state IN ('unknown', 'checking')
                            OR
                            mergeable_state IS NULL
                            OR
                            mergeable IS NULL
                        )
                    )
            ...
            # Repository.fetch_closed_issues_without_closed_by
            sql = """
            CREATE INDEX closed_without_closed_by
            ON core_issue(
                closed_at,
                repository_id,
                state,
    ...
    def backwards(self, orm):
        if not db.dry_run and db.backend_name == 'postgres':
            # Repository.fetch_updated_prs
            sql = "drop index prs_to_update;"
            db.execute(sql)

            # Repository.fetch_closed_issues_without_closed_by
            sql = "drop index closed_without_closed_by;"
            db.execute(sql)
