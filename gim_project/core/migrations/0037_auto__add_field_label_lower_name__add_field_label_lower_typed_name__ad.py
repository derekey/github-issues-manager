...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'Label.lower_name'
        db.add_column(u'core_label', 'lower_name',
                      self.gf('django.db.models.fields.TextField')(default='', db_index=True),
                      keep_default=False)

        # Adding field 'Label.lower_typed_name'
        db.add_column(u'core_label', 'lower_typed_name',
                      self.gf('django.db.models.fields.TextField')(default='', db_index=True),
                      keep_default=False)

        # Adding field 'LabelType.lower_name'
        db.add_column(u'core_labeltype', 'lower_name',
                      self.gf('django.db.models.fields.CharField')(default='', max_length=250, db_index=True),
                      keep_default=False)

        # Removing index on 'LabelType', fields ['name']
        # db.delete_index(u'core_labeltype', ['name'])

        if not db.dry_run:
            for label_type in orm.LabelType.objects.all():
                label_type.lower_name = label_type.name.lower()
                label_type.save()  # dont_update_labels=True)
            for label in orm.Label.objects.all():
                label.lower_name = label.name.lower()
                label.lower_typed_name = label.typed_name.lower()
                label.save()  # dont_update_label_type=True)


    def backwards(self, orm):
        # Adding index on 'LabelType', fields ['name']
        db.create_index(u'core_labeltype', ['name'])

        # Deleting field 'Label.lower_name'
        db.delete_column(u'core_label', 'lower_name')

        # Deleting field 'Label.lower_typed_name'
        db.delete_column(u'core_label', 'lower_typed_name')

        # Deleting field 'LabelType.lower_name'
        db.delete_column(u'core_labeltype', 'lower_name')
    ...
    models = {
        ...
        u'core.label': {
            'Meta': {'ordering': "('label_type', 'order', 'lower_typed_name')", 'unique_together': "(('repository', 'name'),)", 'object_name': 'Label', 'index_together': "(('repository', 'label_type', 'order'),)"},
