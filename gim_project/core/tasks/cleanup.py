...
def clean_errors(print_step=1000, keep_days=7, batch_size=100):
    ...
    iterate = 0
    while not stop:
        instances = list(Error.collection().sort(by='date', alpha=True).instances()[:batch_size])
        if not instances:
            break
        for instance in instances:
            ...
            try:
                if instance.date.hget() >= max_date:
                    stop = True
                    break
                instance.delete()
            except Exception, e:
...
def clean_job_model(job_model, print_step=1000, keep_days=7, batch_size=100):
    ...
    iterate = 0
    while True:
        instances = list(job_model.collection().instances()[kept:kept+batch_size])
        if not instances:
            break
        for instance in instances:
            iterate += 1
            ...
            try:
                if instance.ident in planned or instance.added.hget()[:10] >= max_date:
