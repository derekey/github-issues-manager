...
class JobMixin(object):
    ...
    def add_job(cls, identifier, priority=0, queue_model=None, prepend=False, **fields_if_new):
        return super(JobMixin, cls).add_job(identifier, cls.queue_name, priority, queue_model, prepend, **fields_if_new)


class DjangoModelJobMixin(JobMixin):
    """
...
class DelayableJob(LimpydJob):
    ...
    def add_job(cls, identifier, queue_name, priority=0, queue_model=None, prepend=False, **fields_if_new):
        ...
        return super(DelayableJob, cls).add_job(identifier, queue_name, priority, queue_model, prepend, **fields_if_new)
